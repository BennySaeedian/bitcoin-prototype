...
def test_block_mining(alice: Node) -> None:
    ...
    assert alice.get_balance() == NUM_OF_COINBASE_PER_BLOCK
    assert len(alice.get_utxo()) == NUM_OF_COINBASE_PER_BLOCK
    assert alice.get_mempool() == []
    ...
    assert len(transactions) == NUM_OF_COINBASE_PER_BLOCK
...
def test_moving_funds_and_balances(alice: Node, bob: Node) -> None:
    ...
    assert bob.get_balance() == 0
    bob.mine_block()
    assert bob.get_balance() == NUM_OF_COINBASE_PER_BLOCK
    assert bob.get_mempool() == []
    alice.mine_block()
    assert alice.get_mempool() == []
    assert alice.get_balance() == NUM_OF_COINBASE_PER_BLOCK
    assert bob.get_balance() == NUM_OF_COINBASE_PER_BLOCK + 1
